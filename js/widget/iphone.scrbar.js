...
(function($) {
	$.fn.extend({
		scrbar: function(option) {	
			var _this=$(this);
			var _cont,_panel,_bar,_tar,_can,_size,_sizeLast,_timer,_posLast=[],_static=false,_speed=1,_barSize;
			var _onTop,_onEnd;
			if(option){
			...
			function scrollFunc(){
				...
				_tar=_tar<0?0:_tar;
				_bar.css({y:_tar});
				_cont.css({y:-_tar/(_barSize)*(_size-_this.height())});	
				if(_onTop && _tar==0) _onTop();
				else if(_onEnd && _tar==_barSize) _onEnd();
			}//end func			
		},//end fn
